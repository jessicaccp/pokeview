...
function PokemonList() {
  ...
  const perPageOptions = [12, 24, 48, 96, 192, 384, count];

  const perPageOptionsList = perPageOptions.map((option, key) => (
  ...
  const selectOptionsList = selectOptions.map((option, key) => (
        ...
        if (count !== 0) {
          data.results.forEach((result) => {
            fetch(`${result.url}`)
              ...
              .then((resultData) =>
                setPokemonData((prev) => [...prev, resultData])
              );
          });
        }
        setCount(data.count);
  ...
  function setPageContent() {
    // pageContent = '';
    pageContent = pokemonData.map((pokemon) => (
      <PokemonCard
        key={pokemon.id}
        id={pokemon.id}
        name={pokemon.name}
        ...
        stats={pokemon.stats.map((stat, key) => (
          <li key={key}>
            {stat.stat.name}: {stat.base_stat}
          </li>
        ))}
        types={pokemon.types.map((type, key) => (
          <li key={key}>{type.type.name}</li>
        ))}
      />
    ));

    pageContent.sort(function (a, b) {
      let c, d;
      switch (order) {
        case "id":
          c = a.props.id;
